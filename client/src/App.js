    ...
    persons.filter(p => {
      ...
      return(
        <div key={person.name} className="person">
          ...
          <Button className="delButton" text="delete" handleClick={() => {
...
const Form = ({addName, newName, newNumber, onNameChange, onNumberChange}) => {
  return(
    <form onSubmit={addName} className="form">
      ...
      <div className="f">
        <input placeholder="number" value={newNumber} onChange={onNumberChange}/>
      </div>
...
const App = () => {
  ...
  const addName = (event) => {
    ...
    if(persons.find(person => person.name === newName) 
      ...
      if(window.confirm(`${newName} is already there, wanna change the number to the new one?`)){
        noteService.updatePerson(newName, newNumber, setNotif, notif)
        const copy2 = [...persons]
        copy2.map(per => per.name === newName ? per.number = newNumber : void(0))
        setPersons(copy2)
    }
    }else{
  ...
  return (
    <div>
      ...
      <h3>New number:</h3>
      <Form addName={addName} newName={newName} newNumber={newNumber} onNameChange={onNameChange} onNumberChange={onNumberChange}/>
      <h3 className="h3">Numbers</h3>
      <RenderPersons persons={persons} search={search} setPersons={setPersons} notif={notif} setNotif={setNotif}/>
    </div>
  )
}
